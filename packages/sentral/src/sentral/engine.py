"""Machine-native agent execution engine."""
...
from sentral.interfaces import Compactor, ConversationStore, Provider, Machine, ToolRegistry
...
class AgentRuntimeEngine(Machine):
    ...
    def __init__(
        ...
        *,
        ...
        self._tool_semaphore = asyncio.Semaphore(8)
    ...
    async def run_turn(
        ...
        *,
        ...
        logger.debug(
            ...
            (
                getattr(request.config, "stream", None)
                if getattr(request, "config", None) is not None
                else None
            ),
            (
                getattr(request.config, "max_iterations", None)
                if getattr(request, "config", None) is not None
                else None
            ),
        ...
        result, _events = await self._execute(
            request, history=history, sink=sink, checkpoint=checkpoint
        )
    ...
    async def stream_turn(
        ...
        *,
        ...
        async def _run() -> None:
            ...
            try:
                result, _events = await self._execute(
                    request, history=history, sink=_sink, checkpoint=checkpoint
                )
                await self._persist_history(request, result)
    ...
    async def _execute(
        ...
        *,
        ...
        async def _drain_injected_items() -> bool:
            if interjection_source is None or pending_tools:
                return False
        ...
        async def _collect_pending(*, wait=False):
            if wait and pending_tools:
                await asyncio.gather(
                    *(task for _, task in pending_tools.values()), return_exceptions=True
                )
            for call_id, (call, task) in list(pending_tools.items()):
                if not task.done():
                    continue
                ...
                result = self._normalize_tool_execution_results([call], [value])[0]
                ...
                approval = self._approval_from_tool_result(result)
                if approval is not None:
                    await _emit(AgentEvent(type="approval_required", approval_request=approval))
                    await _emit(AgentEvent(type="done", stop_reason="pending_approval"))
                    return False
            return True

        async def _run_iterations():
            try:
                await _run_iterations_inner()
            finally:
                if pending_tools:
                    for _, task in pending_tools.values():
                        if not task.done():
                            task.cancel()
                    await _collect_pending(wait=True)

        async def _run_iterations_inner() -> None:
            nonlocal iterations
            auto_steps = config.max_iterations == 0
            ...
            for index in count():
                if not auto_steps and index >= total_limit:
                    break
                if not await _collect_pending():
                    return
                await _drain_injected_items()
                if not auto_steps and index == effective_max and not grace_check_done:
                    ...
                    if not await self._grace_analysis(
                        working_history, effective_max, grace_extension
                    ):
                ...
                iterations = index + 1
                progress_max = (
                    0 if auto_steps else (total_limit if grace_check_done else effective_max)
                )
                ...
                if turn.stop_reason == "pause_turn":
                    continue
                if turn.stop_reason != "tool_use" and pending_tools:
                    if not await _collect_pending(wait=True):
                        return
                    continue
                if turn.stop_reason == "stop" and await _drain_injected_items():
                    continue
                ...
                tool_calls = [
                    block for block in assistant_item.content if isinstance(block, ToolCallBlock)
                ]
                if not tool_calls:
                ...
                try:
                    if effective_tool_calls:
                        ...
                        async_ids = {
                            item.get("call_id")
                            for item in assistant_item.metadata.get("responses_output", [])
                            if item.get("type") == "function_call" and item.get("async") is True
                        }
                        synchronous = []
                        for call in effective_tool_calls:
                            if call.id in async_ids and call.name not in {"form", "sub_agents"}:
                                if call.id not in pending_tools:
                                    pending_tools[call.id] = (
                                        call,
                                        asyncio.create_task(self._execute_one_tool(call)),
                                    )
                            else:
                                synchronous.append(call)
                        if synchronous:
                            tool_results.extend(await self._execute_tool_calls(synchronous))
                        logger.debug(
                ...
                for result in tool_results:
                    ...
                    if approval_request is not None:
                        ...
                        return
                for result in tool_results:
                    if result.tool_name == "form" and not result.is_error:
                        try:
                            form_result = json.loads(result.content)
                        except (ValueError, TypeError):
                            continue
                        if (
                            isinstance(form_result, dict)
                            and form_result.get("kind") == "form"
                            and form_result.get("status") == "awaiting_input"
                        ):
                            await _emit(AgentEvent(type="done", stop_reason="awaiting_input"))
                            return
                if tool_execution_cancelled:
                    logger.debug(
                        "iteration_aborted_after_tool_cancellation iteration=%d", iterations
                    )
        ...
        if isinstance(timeout_seconds, (int, float)) and float(timeout_seconds) > 0:
            try:
                ...
                logger.debug(
                    "execute_timeout timeout_seconds=%s iterations=%d", timeout_seconds, iterations
                )
        ...
        logger.debug(
            ...
            (
                ...
                else ("error" if last_stop_reason == "error" else "completed")
            ),
            iterations,
            last_stop_reason,
        ...
        *,
        ...
        if config.stream:
            return await self._stream_response(
                history, tools=tool_schemas, config=config, emit=emit
            )
    ...
    async def _stream_response(
        ...
        *,
        ...
        return self._assemble_turn_from_events(
            streamed_events, fallback_model=config.model, fallback_provider=self._provider.name
        )
    ...
    def _assemble_turn_from_events(
        ...
        *,
        ...
        for index, event in enumerate(events):
            idx = (
                int(event.metadata.get("content_index", index))
                if isinstance(event.metadata.get("content_index"), int)
                else index
            )
            if event.item is not None and event.item.role == "assistant":
                if event.type == "done":
                    responses_output = deepcopy(event.item.metadata.get("responses_output") or [])
                    responses_context_reset = bool(
                        event.item.metadata.get("responses_context_reset")
                    )
                    provider_usage = deepcopy(event.item.metadata.get("provider_usage"))
                model = str(event.item.metadata.get("model") or model)
                provider = str(event.item.metadata.get("provider") or provider)
                usage_payload = (
                    event.item.metadata.get("usage")
                    if isinstance(event.item.metadata.get("usage"), dict)
                    else {}
                )
        ...
        for block_type, idx in block_sequence:
            if block_type == "text":
                ...
                if raw_args:
                    ...
                    parsed_args = (
                        state.get("base_args") if isinstance(state.get("base_args"), dict) else {}
                    )
                ...
                content.append(
                    ToolCallBlock(
                        ...
                        thought_signature=(
                            ...
                            if isinstance(state.get("thought_signature"), str)
                            and str(state.get("thought_signature")).strip()
                            else None
                        ),
                    )
                )
        ...
        item = ConversationItem(
            ...
            metadata={
                ...
                "responses_context_reset": responses_context_reset,
    ...
    async def _execute_tool_calls(self, tool_calls: list[ToolCallBlock]) -> list[ToolResultBlock]:
        ...
        try:
            ...
            raise _ToolExecutionCancelled(
                self._normalize_tool_execution_results(tool_calls, results)
            ) from None
    ...
    def _approval_from_tool_result(self, result: ToolResultBlock) -> ApprovalRequest | None:
        ...
        return ApprovalRequest(
            ...
            tool_name=str(
                approval.get("tool_name") or approval.get("provider") or result.tool_name
            ).strip(),
            ...
            metadata={
                ...
                if key
                not in {"id", "approval_id", "tool_name", "provider", "action", "description"}
            },
        )
        ...
        *,
        ...
        for block in final_turn.item.content:
            if isinstance(block, TextBlock) and block.text:
                await emit(AgentEvent(type="text_delta", delta=block.text))
        await emit(
            AgentEvent(type="done", stop_reason=final_turn.stop_reason, item=final_turn.item)
        )
        logger.debug(
    ...
    async def _grace_analysis(
                ...
                for block in item.content:
                    if isinstance(block, ToolResultBlock):
                        ...
                        summary_lines.append(
                            f"tool_result({status}): {(block.content or '')[:300]}"
                        )
        ...
        try:
            ...
            text = "\n".join(
                block.text
                for block in response.item.content
                if isinstance(block, TextBlock) and block.text
            ).strip()
            if text.startswith("```"):
...
def _summarize_content_blocks(blocks: list[Any]) -> str:
    ...
    for block in blocks:
        if isinstance(block, TextBlock):
            ...
            parts.append(
                f"tool_result(name={block.tool_name},error={block.is_error},len={len(block.content or '')})"
            )
...
def _summarize_event(event: AgentEvent) -> str:
    ...
    if event.metadata:
        ...
        interesting = {
            k: event.metadata[k]
            for k in sorted(event.metadata)
            if k in {"content_index", "provider", "model", "signature"}
        }
        if interesting:
