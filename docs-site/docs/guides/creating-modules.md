...
| Type | Behavior |
|---|---|
| `data` | Record CRUD on `/api/v1/instances/{instance_name}/modules/{name}/records` plus optional custom actions |
...
```http
POST /api/v1/instances/{instance_name}/modules/{name}/records/{id}/action/{action_id}
```
...
1. `GET /api/v1/instances/{instance_name}/modules/{name}` returns expected schema
