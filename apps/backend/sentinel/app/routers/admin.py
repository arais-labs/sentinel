...
async def list_audit_logs(
    ...
    return AuditLogListResponse(items=[_audit_response(item) for item in paged], total=len(items))
...
async def get_config(
    ...
    _ = user
    return _config_response(estop_active=await is_estop_active(db))
...
def _audit_response(item: AuditLog) -> AuditLogResponse:
    timestamp = item.timestamp or datetime.now(UTC)
    ip_address: str | None
    if item.ip_address is None:
        ip_address = None
    else:
...
async def is_estop_active(db: AsyncSession) -> bool:
    return (await _estop.check_level(db)) != EstopLevel.NONE


async def set_estop_active(db: AsyncSession, *, active: bool) -> None:
    level = EstopLevel.TOOL_FREEZE if active else EstopLevel.NONE
    await _estop.set_level(db, level)


def _config_response(*, estop_active: bool) -> ConfigResponse:
