...
class BrowserManager:
    ...
    async def navigate(self, url: str) -> dict[str, Any]:
        ...
        await page.goto(
            normalized, wait_until="domcontentloaded", timeout=self._timeout_ms
        )
        title = await page.title()
        current_url = getattr(page, "url", normalized)
        tab_id = self._register_page(page, make_active=True)
        return {"url": current_url, "title": title or "", "tab_id": tab_id}
    ...
    async def screenshot(self, *, full_page: bool = True) -> dict[str, Any]:
        ...
        tab_id = self._register_page(page, make_active=True)
        return {"image_base64": encoded, "tab_id": tab_id}
    ...
    async def click(self, selector: str) -> dict[str, Any]:
        ...
        if semantic is not None:
            role, name = semantic
            await self._click_by_accessible_name(page, role=role, name=name)
            await self._promote_new_tab_if_any(before_page_ids)
            return await self._click_response(resolved)

        if resolved.lower().startswith("aria/"):
            locator = page.locator(f"aria={resolved[5:].strip()}").first
            await self._click_locator_with_overlay_fallback(page, locator)
            await self._promote_new_tab_if_any(before_page_ids)
            return await self._click_response(resolved)
        ...
        selected = await self._try_select_option_by_selector(page, resolved)
        if selected:
            await self._promote_new_tab_if_any(before_page_ids)
            return await self._click_response(resolved)
        ...
        *,
        ...
        if semantic is not None:
            ...
            selected_values = await self._select_option_by_accessible_name(
                page, role=role, name=name, criteria=criteria
            )
            ...
            selected_values = await locator.select_option(
                **criteria, timeout=self._timeout_ms
            )
        elif target_selector.lower().startswith("aria="):
            ...
            selected_values = await page.select_option(
                target_selector, **criteria, timeout=self._timeout_ms
            )
            ...
            try:
                if action == "type":
                    ...
                    result = await self.select_option(
                        selector, value=value, label=label, index=index_value
                    )
                elif action == "click":
                    ...
                    if timeout_raw is not None and (
                        ...
                        raise ValueError(
                            f"steps[{idx}].timeout_ms must be a positive integer"
                        )
                ...
                if verify and action in {"type", "select"}:
                    step_result["verify"] = await self.get_value(selector)
    ...
    async def type_text(self, selector: str, text: str) -> dict[str, Any]:
        ...
        if semantic is not None:
            role, name = semantic
            await self._fill_by_accessible_name(page, role=role, name=name, text=text)
            return {"typed": True, "selector": resolved, "characters": len(text)}

        if resolved.lower().startswith("aria/"):
            await self._fill_by_aria_selector(page, resolved[5:].strip(), text)
            return {"typed": True, "selector": resolved, "characters": len(text)}

        await page.fill(resolved, text, timeout=self._timeout_ms)
        return {"typed": True, "selector": resolved, "characters": len(text)}
    ...
    async def _click_locator_with_overlay_fallback(
        ...
        try:
            await locator.click(timeout=min(self._timeout_ms, 3_000))
            return
        except Exception:
            pass

        await locator.click(timeout=min(self._timeout_ms, 3_000), force=True)

    async def _fill_by_aria_selector(self, page: Any, name: str, text: str) -> None:
        last_exc: Exception | None = None
        for candidate in self._name_candidates(name):
            try:
                await page.locator(f"aria={candidate}").first.fill(
                    text, timeout=self._timeout_ms
                )
                return
            except Exception as exc:  # noqa: BLE001
                last_exc = exc
    ...
    async def _fill_by_accessible_name(
        self, page: Any, *, role: str, name: str, text: str
    ) -> None:
        last_exc: Exception | None = None
        for candidate in self._name_candidates(name):
            try:
        ...
        *,
        ...
        for candidate in self._name_candidates(name):
            try:
                ...
                return await locator.select_option(**criteria, timeout=self._timeout_ms)
            except Exception as exc:  # noqa: BLE001
                last_exc = exc
        if last_exc is not None:
            raise last_exc
    ...
    async def _promote_new_tab_if_any(self, before_page_ids: set[int]) -> None:
        if self._context is None:
            return
    ...
    async def _try_select_option_by_selector(self, page: Any, selector: str) -> bool:
        ...
        try:
            selected_values = await page.select_option(
                select_selector, value=value, timeout=self._timeout_ms
            )
        except Exception:
            return False
