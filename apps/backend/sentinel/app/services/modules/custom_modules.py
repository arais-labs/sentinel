...
from app.services.modules.definitions import ActionDefinition, ModuleDefinition, ParamDefinition
...
RESERVED_CUSTOM_MODULE_COMMANDS = (
    "list_records",
    "get_record",
    "create_records",
    "update_records",
...
def build_custom_module_definition(
    module: Module,
    *,
    permission_levels: dict[str, str] | None = None,
    session_factory: "async_sessionmaker[AsyncSession]",
) -> ModuleDefinition:
    normalized_actions = normalize_custom_module_actions(module.actions or [])
...
def normalize_custom_module_actions(actions: list[Any]) -> list[dict[str, Any]]:
    ...
    for entry in _custom_action_commands(normalized):
        ...
        if action_id in RESERVED_CUSTOM_MODULE_COMMANDS:
...
def build_custom_module_permission_levels(
    *,
    module_name: str,
    ...
    commands = set(RESERVED_CUSTOM_MODULE_COMMANDS)
...
async def sync_custom_module_permissions(
    db: "AsyncSession",
    *,
    module_name: str,
    actions: list[dict[str, Any]],
    ...
    for command, level in levels.items():
        ...
        if action_key in existing_actions:
            continue
        db.add(ModulePermission(action=action_key, level=level))
    await db.commit()
    return levels


async def delete_custom_module_permissions(
    db: "AsyncSession",
    *,
    module_name: str,
) -> None:
...
async def load_custom_module_tool_definitions(
    *,
    session_factory: "async_sessionmaker[AsyncSession]",
) -> list[Any]:
    async with session_factory() as db:
        ...
        permission_result = await db.execute(select(ModulePermission))
    ...
    for module in modules:
        try:
            definition = build_custom_module_definition(
                module,
                permission_levels=permissions_by_module.get(module.name),
                session_factory=session_factory,
            )
            tool_defs.extend(build_module_tools(definition, session_factory=session_factory))
        except Exception:
            logger.exception("tool_registry_skip_custom_module module=%s", module.name)
    return tool_defs
...
def _serialize_record(record: ModuleRecord) -> dict[str, Any]:
...
def _check_required_secrets(module: Module, secrets: dict[str, str]) -> None:
    missing = [
        secret["key"]
        ...
        async with session_factory() as db:
            result = await db.execute(
                select(ModuleRecord).where(
                    ...
                    ModuleRecord.id == record_id.strip(),
                )
            )
    ...
    async def _handler(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        async with session_factory() as db:
            ...
            for data in records_data:
                record = ModuleRecord(id=generate_record_id(), module_name=module_name, data=data)
                db.add(record)
