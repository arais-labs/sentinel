...
path (PATCH /modules/{name}) and the agent path (module_manager action=edit_module) apply edits
...
from app.services.modules.action_executor import validate_action_code
from app.services.modules.custom_modules import normalize_custom_module_actions
...
def extract_updates(body: Any) -> dict[str, Any]:
    ...
    if not updates:
        raise ValueError(
            f"At least one editable module field is required ({', '.join(MODULE_MUTABLE_FIELDS)})"
        )
    return updates
...
def apply_updates(mod: Module, updates: dict[str, Any]) -> None:
...
def fold_ops_into_delta(mod: Module, request: EditModuleRequest) -> ModuleEditDelta:
    ...
    final_actions = normalize_custom_module_actions(actions) if actions_touched else None
    if final_actions is not None:
        previous = {a.get("id"): a.get("code") for a in (mod.actions or [])}
        validate_action_code(
            [
                a
                for a in final_actions
                if a.get("id") not in previous or a.get("code") != previous[a.get("id")]
            ]
        )
