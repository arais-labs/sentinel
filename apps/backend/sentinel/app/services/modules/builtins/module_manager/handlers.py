...
from app.services.modules.action_executor import execute_action, validate_action_code
from app.services.modules.builtins import is_retired_module
...
from app.services.modules.updates import apply_updates, fold_ops_into_delta
...
def _serialize_record(r: ModuleRecord) -> dict[str, Any]:
    d = dict(r.data or {})
    d["id"] = r.id
...
async def _refresh_current_runtime_after_module_change(app_state: Any | None) -> None:
    ...
    context = instance_context.instance_runtime_context_registry.find_by_session_factory(
        session_factory
    )
    if context is not None:
        await instance_context.instance_runtime_context_registry.rebuild_context(
            app_state=app_state,
            context=context,
        )
        return
...
async def handle_create_module(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    actions = normalize_custom_module_actions(list(module_values.get("actions") or []))
    validate_action_code(actions)
    ...
    async with AsyncSessionLocal() as db:
        existing = await db.execute(select(Module).where(Module.name == name))
...
async def _apply_record_data_migration(
    ...
    *,
    ...
    if not renames and not purge_keys:
        return
    result = await db.execute(select(ModuleRecord).where(ModuleRecord.module_name == module_name))
...
async def handle_edit_module(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        apply_updates(mod, delta.updates)
...
async def handle_list_records(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        result = await db.execute(
            select(ModuleRecord)
            .where(ModuleRecord.module_name == module_name)
...
async def handle_update_records(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        result = await db.execute(
            select(ModuleRecord).where(
                ModuleRecord.module_name == module_name,
                ModuleRecord.id.in_(update_ids),
            )
        )
        ...
        updated_records: list[ModuleRecord] = []
...
async def handle_delete_records(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        result = await db.execute(
            select(ModuleRecord).where(
                ModuleRecord.module_name == module_name,
                ModuleRecord.id.in_(record_ids),
            )
        )
        records_by_id = {record.id: record for record in result.scalars().all()}
...
async def handle_run_action(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        sec_result = await db.execute(
            select(ModuleSecret).where(ModuleSecret.module_name == module_name)
        )
