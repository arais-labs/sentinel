...
def _edit_module_parameters_schema() -> dict:
    ...
    # top-level property merge (definitions._build_grouped_parameters_schema). The only new
...
MODULE = ModuleDefinition(
    ...
    description=(
        "Unified dynamic module engine for module CRUD, record CRUD, and action execution. "
        "Custom action Python supports top-level await. Injected http is an async client; "
        "await its requests. Set result to a dictionary (or an awaitable resolving to one). "
        "Example: response = await http.get('https://api.example.com/items', "
        "headers={'Authorization': 'Bearer ' + secrets['API_TOKEN']}); "
        "response.raise_for_status(); result = {'items': response.json()}. "
        "Code is syntax-checked before saving; validation does not execute requests."
    ),
    icon="boxes",
    system=True,
    grouped_tool=True,
    actions=[
        ...
        ActionDefinition(
            ...
            description=(
                ...
                "To CHANGE an EXISTING module, use action=edit_module — never delete+recreate "
