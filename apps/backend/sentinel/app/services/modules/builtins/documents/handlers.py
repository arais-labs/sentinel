...
from app.models.modules import ModuleRecord, generate_record_id

_MODULE = "documents"


def _serialize(r: ModuleRecord) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as db:
        rows = (
            (
                await db.execute(
                    select(ModuleRecord)
                    .where(ModuleRecord.module_name == _MODULE)
...
async def handle_get(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as db:
        rows = (
            (await db.execute(select(ModuleRecord).where(ModuleRecord.module_name == _MODULE)))
            .scalars()
            .all()
        )
    for r in rows:
...
async def handle_create(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        record = ModuleRecord(id=generate_record_id(), module_name=_MODULE, data=data)
        db.add(record)
        await db.commit()
            ...
            (
                await db.execute(
                    select(ModuleRecord).where(
                        ModuleRecord.module_name == _MODULE,
                        ModuleRecord.id == doc_id,
                    )
                )
            )
            .scalars()
