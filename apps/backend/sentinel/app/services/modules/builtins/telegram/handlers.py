...
from app.services.telegram import lifecycle
...
# ---------------------------------------------------------------------------
# Helpers scoped to the acting instance
# ---------------------------------------------------------------------------
...
async def _rebuild_instance_context(
...
async def _resolve_owner_user_id_from_session(
    ...
    return await lifecycle.resolve_owner_user_id_from_session(context.session_factory, session_id)


async def _persist_telegram_settings(context: InstanceRuntimeContext, **kwargs: Any) -> None:

    await lifecycle.persist_telegram_settings(context.session_factory, **kwargs)


def _mask_telegram_token(value: str | None) -> str | None:

    return lifecycle.mask_telegram_token(value)
...
async def handle_send(payload: dict[str, Any], runtime: ToolRuntimeContext) -> dict[str, Any]:
    ...
    if ok:
        return {
            "success": True,
            "chat_id": chat_id,
            "message_sent": message.strip()[:200],
        }
...
async def _status_payload(context: InstanceRuntimeContext) -> dict[str, Any]:
    ...
    owner_user_id = instance_settings.telegram_owner_user_id or "local"
    return {
