...
class ContextBuilder:
    ...
    async def _recent_messages_within_budget(
        ...
        *,
        ...
        fixed_tokens = self._estimate_context_tokens(fixed_context) + self._estimate_text_tokens(
    ...
    def _estimate_context_tokens(self, messages: list[AgentMessage]) -> int:
        return sum(self._estimate_agent_message_tokens(message) for message in messages)
        ...
        if isinstance(message, UserMessage):
            ...
            for block in message.content:
                if isinstance(block, TextContent):
                    total += self._estimate_text_tokens(block.text)
                elif isinstance(block, ImageContent):
                    total += self._estimate_text_tokens(block.data)
            return total
        if isinstance(message, AssistantMessage):
            ...
            for block in message.content:
                if isinstance(block, TextContent):
                    ...
                    total += self._estimate_text_tokens(block.name)
                    total += self._estimate_text_tokens(str(block.arguments or {}))
            return total
        return 0
