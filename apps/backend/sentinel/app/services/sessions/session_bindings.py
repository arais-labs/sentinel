...
async def bind_session(
    ...
    *,
    ...
    session = await _get_root_session(db, user_id=user_id, session_id=session_id)
    if session is None:
        raise SessionBindingTargetInvalidError(
            "Binding target must be a root session in this workspace"
        )
...
async def set_owner_active_session(
    ...
    *,
    ...
    if session is None:
        raise SessionBindingTargetInvalidError(
            "Active session must be a root session in this workspace"
        )
...
async def _get_root_session(db: AsyncSession, *, user_id: str, session_id: UUID) -> Session | None:
    result = await db.execute(
