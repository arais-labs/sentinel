...
class TriggerScheduler:
    ...
    async def _resolve_or_create_main_session(
        ...
        *,
        ...
        logger.info(
            "Resolved trigger target main session for '%s': %s", trigger_name, session.id
        )
        return session
    ...
    async def _is_allowed_trigger_session_target(
        ...
        *,
        ...
        if candidate_session.id == main_session_id:
            return True
        return await self._is_telegram_route_session(db, user_id=user_id, session_id=candidate_session.id)
    ...
    async def _is_telegram_route_session(
        ...
        *,
        ...
        bound = await session_bindings.is_session_bound(
            db,
            user_id=user_id,
