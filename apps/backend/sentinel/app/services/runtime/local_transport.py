...
class _PTYReader(_StreamReader):
    async def read(self, n: int = -1) -> str | bytes:
        try:
...
class _PTYWriter:
    """Write PTY input without treating its bidirectional fd as a pipe."""
    ...
    def write(self, data: bytes) -> None:
        if self._fd is None:
            raise BrokenPipeError("Terminal viewer is closed")
...
class _LocalPTYProcess(_LocalProcess):
    def __init__(self, process, master, reader, reader_transport, writer_transport, encoding):
        super().__init__(process, encoding)
        self._master = master
        self._reader_transport = reader_transport
        self._writer_transport = writer_transport
        self.stdout = _PTYReader(reader, encoding)
        self.stdin = _StreamWriter(writer_transport, encoding)
...
class LocalTransport:
    ...
    build_shell_command wrapper the SSH path uses. This transport serves machine
    inspection and folder selection; agent commands use ContainerTransport.
    """
    ...
    async def create_process(
        ...
        *,
        ...
        if term_type:
            ...
            reader_transport = writer_transport = None
            process = None
            try:
                cols, rows = term_size
                ...
                process = await asyncio.create_subprocess_shell(
                    ...
                    stdout=slave,
                    stderr=slave,
                    env={**await self._environment(), "TERM": term_type},
                    start_new_session=True,
                )
                loop = asyncio.get_running_loop()
                reader = asyncio.StreamReader(limit=_TERMINAL_STREAM_LIMIT)
                reader_transport, _ = await loop.connect_read_pipe(
                    lambda: asyncio.StreamReaderProtocol(reader),
                    os.fdopen(master, "rb", buffering=0),
                )
                writer_transport = _PTYWriter(os.dup(master))
                return _LocalPTYProcess(
                    process, master, reader, reader_transport, writer_transport, encoding
                )
