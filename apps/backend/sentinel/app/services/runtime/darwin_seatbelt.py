...
def _path_aliases(path: str) -> list[str]:
    aliases = [path]
    for source, target in (("/tmp", "/private/tmp"), ("/var", "/private/var"), ("/etc", "/private/etc")):
...
def build_seatbelt_profile(paths: RemoteWorkspacePaths) -> str:
    ...
    lines.extend(f"  (subpath {_sb_string(path)})" for path in _unique(system_read_roots + executable_roots))
    lines.extend(
        [
            ")",
            "(allow file-read* file-test-existence",
    ...
    lines.extend(f"  (subpath {_sb_string(path)})" for path in _unique(system_read_roots + executable_roots + session_paths))
    lines.extend([")"])
...
def build_seatbelt_command(paths: RemoteWorkspacePaths, profile_path: str, command: list[str]) -> str:
    sandbox_command = " ".join(["sandbox-exec", "-f", quote(profile_path), *(quote(part) for part in command)])
    return " ".join(
        [
            "/bin/sh",
            "-c",
