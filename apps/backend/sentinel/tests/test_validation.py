...
def test_validation_hardening_rules():
    ...
    try:
        ...
        invalid_session = client.post("/api/v1/sessions", json={"title": too_long_title}, headers=headers)
        assert invalid_session.status_code == 422

        valid_session = client.post("/api/v1/sessions", json={"title": "  Trimmed Title  "}, headers=headers)
        ...
        invalid_playwright = client.post(
            "/api/v1/playwright/tasks",
            json={"url": "example.com", "action": "screenshot"},
            headers=headers,
        )
        assert invalid_playwright.status_code == 422
