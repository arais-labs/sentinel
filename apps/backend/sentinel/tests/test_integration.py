...
def test_full_integration_happy_path():
    ...
    try:
        ...
        created_session = client.post("/api/v1/sessions", json={"title": "integration-e2e"}, headers=headers)
        ...
        cancelled = client.delete(f"/api/v1/sessions/{session_id}/sub-agents/{task_id}", headers=headers)
        ...
        playwright_created = client.post(
            "/api/v1/playwright/tasks",
            json={"url": "https://example.com", "action": "screenshot"},
            headers=headers,
        )
        assert playwright_created.status_code == 200
        playwright_id = playwright_created.json()["id"]

        playwright_detail = client.get(f"/api/v1/playwright/tasks/{playwright_id}", headers=headers)
        assert playwright_detail.status_code == 200

        playwright_cancel = client.delete(f"/api/v1/playwright/tasks/{playwright_id}", headers=headers)
        assert playwright_cancel.status_code == 200
        ...
        blocked_playwright = client.post(
            "/api/v1/playwright/tasks",
            json={"url": "https://example.com", "action": "extract"},
            headers=headers,
        )
        assert blocked_playwright.status_code == 403
