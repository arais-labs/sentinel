...
from app.services.memory_search import MemorySearchResult, MemorySearchService
...
def test_context_builder_injects_all_root_memories_and_auto_branches():
    ...
    roots_block = next(msg for msg in system_messages if "## Root Memories" in msg)
    assert str(root_a.id) in roots_block
    assert str(root_b.id) in roots_block
    assert "All root memories are injected below." in roots_block
