...
//   node scripts/publishing/publish-release.mjs <channel>
...
async function main() {
  ...
  const prerelease = `--prerelease=${channel === 'beta'}`;

  const dmgPath = await findDist(
    (name) => name === `Sentinel-${version}-arm64.dmg`,
    `DMG for ${version}`,
  );
  ...
  if (releaseExists(versionedTag)) {
    ...
    run('gh', ['release', 'edit', versionedTag, prerelease, '--repo', repoSlug]);
  } else {
    run('gh', [
