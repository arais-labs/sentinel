...
// shell DMG (which only carries python/git/gh). At runtime the shell
...
async function stageSitePackages(stagingDir) {
  ...
  run('uv', [
    ...
    '--no-dev',
    '--no-emit-project',
    '--no-emit-package', 'sentral-runtime',
  ...
  // Install shared source as a normal wheel, never an editable path into the
  // build checkout. Third-party dependencies above retain lockfile hashes.
  run(bundledPython, [
    '-m', 'pip', 'install',
...
async function stageBackend(stagingDir) {
  ...
  // tar pipe copy with excludes: preserves Alembic migrations while dropping
  // dev/venv/cache cruft and anything that
  // could carry developer identity or secrets.
  const excludes = [
    '.venv', '__pycache__', '*.pyc', '*.egg-info', 'tests', '.pytest_cache',
    'node_modules', '.git', '.env', '.env.*', 'uv.lock', '._*', '.DS_Store',
  ];
  run('bash', ['-c',
...
async function buildPayload() {
  ...
  // AppleDouble sidecars look like Python migrations to Alembic. Never ship
  // macOS resource-fork metadata in this cross-platform application payload.
  run('tar', ['-czf', tarPath, '--exclude=._*', '--exclude=.DS_Store', '-C', stagingDir, '.'], {
    env: { COPYFILE_DISABLE: '1' },
  });
