  ...
  return [
    {
      ...
      inputs: [
        ...
        path.join(backendDir, 'alembic.manager.ini'),
        path.join(backendDir, 'alembic.instance.ini'),
        path.join(backendDir, 'app'),
        path.join(backendDir, 'db/alembic'),
      ],
...
async function buildBackendRuntime({ config, paths }) {
  ...
  run('uv', [
    ...
    '--add-data',
    `${path.join(backendDir, 'alembic.manager.ini')}:alembic.manager.ini`,
    '--add-data',
    `${path.join(backendDir, 'alembic.instance.ini')}:alembic.instance.ini`,
    '--add-data',
    `${path.join(backendDir, 'db/alembic')}:db/alembic`,
