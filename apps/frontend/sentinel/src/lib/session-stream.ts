import { DesktopSocket } from './desktop-socket';
...
interface StreamEntry {
  ...
  runListeners: Set<(active: boolean) => void>;
...
function connect(entry: StreamEntry): void {
  ...
  ws.onopen = () => {
    ...
    // Transport open is not session readiness. Wait for the connected payload.
  };
  ...
  ws.onmessage = (messageEvent) => {
    ...
    try {
      ...
      if (payload.type === 'connected') {
        entry.lastConnected = payload;
        entry.reconnectAttempts = 0;
      }
      if (typeof payload.run_active === 'boolean') {
        entry.runActive = payload.run_active;
        for (const listener of entry.runListeners) listener(entry.runActive);
      }
      if (payload.type === 'connected') setConnection(entry, 'connected');
...
export interface SessionStreamHandlers {
  ...
  onRunState?: (active: boolean) => void;
...
export function subscribeSessionStream(
  ...
  if (handlers.onRunState) {
    entry.runListeners.add(handlers.onRunState);
    handlers.onRunState(entry.runActive);
  }
  ...
  return {
    ...
    unsubscribe: () => {
      ...
      if (handlers.onRunState) activeEntry.runListeners.delete(handlers.onRunState);
      ...
      const hasSubscribers =
        ...
        activeEntry.runListeners.size > 0 ||
...
/** Read the latest server run state, including updates React has not rendered yet. */
export function isSessionRunActive(instanceName: string, sessionId: string): boolean {
  return entries.get(streamKey(instanceName, sessionId))?.runActive ?? false;
}
