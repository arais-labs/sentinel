...
/**
 * App-level instance switcher for the left sidebar. Present in
 ...
 * route. Clicking the Sentinel logo returns to instance management.
 */
  ...
  return (
    <div className="px-2 pt-3 space-y-1">
      <button
        ...
        className="group flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-(--text-secondary) hover:bg-(--surface-1) hover:text-(--text-primary) transition-colors"
      >
        <Server
          size={18}
        ...
        <ChevronDown
          size={14}
          aria-hidden="true"
          className={`shrink-0 text-(--text-muted) transition-[transform,opacity] duration-200 ${
            expanded ? 'opacity-100' : 'opacity-0 pointer-events-none'
          } ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>
      ...
      {isOpen && menuRect &&
        createPortal(
          <div
            ...
            className="fixed z-10000 max-h-80 overflow-y-auto rounded-2xl border border-(--border-strong) bg-(--surface-0) py-0 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 duration-150 origin-top-left"
          >
            {instances.length === 0 ? (
              <div className="px-3 py-3 text-xs text-(--text-muted)">No instances</div>
            ) : (
                  ...
                  >
                    <div
                      className={`h-1.5 w-1.5 shrink-0 rounded-full ${
                        active
                          ? 'bg-(--accent-solid)'
                          : 'bg-(--text-muted)/35 group-hover:bg-(--text-secondary)'
                      }`}
                    />
                    <span className="min-w-0 flex-1 truncate text-xs font-semibold">{title}</span>
                    {active && <Check size={13} className="shrink-0 text-(--accent-solid)" />}
                  </button>
                );
              })
            )}
