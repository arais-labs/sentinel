...
export const Workbench: React.FC<WorkbenchProps> = ({
  ...
  return (
    <aside
      ...
      className={`relative z-30 flex h-full flex-col border-l border-(--border-subtle) bg-(--surface-1) overflow-hidden min-w-[400px] animate-[workbenchDockIn_180ms_ease-out] ${className}`.trim()}
    >
      <div className="flex flex-1 min-h-0">
        {showExplorer ? (
          explorerVisible ? (
            ...
            <button
              ...
              title="Show explorer"
              className="w-9 shrink-0 flex items-start justify-center pt-3 border-r border-(--border-subtle) bg-(--surface-1) text-(--text-muted) hover:text-(--text-primary) hover:bg-(--surface-2) transition-colors"
            >
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-(--surface-0)">
          {/* Tabs Bar */}
          <div className="relative h-12 border-b border-(--border-subtle) bg-(--surface-1) flex items-center overflow-x-auto no-scrollbar pr-12">
            {tabs.map(tab => (
              <div
                key={tab.path}
                className={`group h-full flex items-center gap-2 px-3 border-r border-(--border-subtle) min-w-[120px] max-w-[200px] cursor-pointer transition-colors relative ${
                  activeTabPath === tab.path
                    ? 'bg-(--surface-0) text-(--text-primary) after:absolute after:bottom-0 after:left-0 after:right-0 after:h-[2px] after:bg-(--accent-solid)'
                    : 'text-(--text-muted) hover:bg-(--surface-2)'
                }`}
                onClick={() => onTabClick(tab.path)}
              >
                <FileCode2 size={14} className={activeTabPath === tab.path ? 'text-(--accent-solid)' : 'opacity-60'} />
            ...
            {tabs.length === 0 && (
              <div className="px-4 text-[10px] text-(--text-muted) italic">
                No open files
              </div>
            )}
            <button
              onClick={onCloseAll}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-(--text-muted) hover:text-rose-500 transition-colors"
          ...
          {activeTab ? (
            <div className="flex-1 flex flex-col min-h-0">
              {/* Toolbar */}
              <div className="p-2 border-b border-(--border-subtle) bg-(--surface-1) flex items-center justify-between gap-4">
                <div className="flex items-center gap-1 rounded-lg border border-(--border-subtle) p-0.5 bg-(--surface-2)">
                  <button
                    onClick={() => setDiffMode(false)}
                    className={`px-3 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${
                      !diffMode ? 'bg-(--surface-0) text-(--text-primary) shadow-xs' : 'text-(--text-muted) hover:text-(--text-secondary)'
                    }`}
                  >
                    Content
                  </button>
                ...
                {diffMode && (
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-(--text-muted) uppercase tracking-tight">Base Ref:</span>
                    <select
                      value={diffBaseRef}
                      onChange={(e) => onDiffBaseRefChange(e.target.value)}
                      className="h-7 rounded-md border border-(--border-subtle) bg-(--surface-0) px-2 text-[10px] font-mono text-(--text-secondary) focus:outline-hidden focus:ring-1 focus:ring-(--accent-solid)"
                    >
              ...
              <div className="flex-1 min-h-0 relative">
                {diffMode ? (
                  diffLoading ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-(--surface-0)/50 backdrop-blur-[1px] z-10">
                      <Loader2 size={24} className="animate-spin text-(--accent-solid)" />
                      <span className="text-[11px] font-medium text-(--text-secondary)">Loading Git Diff...</span>
                    </div>
                  ) : diffError ? (
                    ...
                    <div className="h-full flex flex-col animate-in fade-in slide-in-from-top-2 duration-300">
                      <div className="px-4 py-2 flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-(--text-muted) border-b border-(--border-subtle) bg-(--surface-1)">
                        <div className="flex items-center gap-2">
                          <GitBranch size={12} />
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-(--text-muted) p-12 text-center">
              <div className="w-16 h-16 rounded-3xl bg-(--surface-2) flex items-center justify-center mb-6">
                <Files size={32} strokeWidth={1} />
              </div>
              <h3 className="text-[13px] font-bold text-(--text-primary) mb-2 uppercase tracking-widest">Workbench</h3>
